...
const InvestorDashboard = () => {
  ...
  const totalIncome = monthlyData.reduce((sum, m) => sum + m.income, 0).toFixed(2);
  ...
  const exportCSV = () => {
    ...
    link.download = `IWB_InvestorIncome_${year}.csv`;
    link.click();
  };
  ...
  const exportPDF = () => {
    const doc = new jsPDF();
    doc.setFontSize(16);
    doc.text(`📄 IWB Investor Income Overview - ${year}`, 14, 20);
    doc.autoTable({
      startY: 30,
      head: [['Month', 'Income (M)']],
      body: monthlyData.map(row => [row.month, row.income.toFixed(2)]),
    });
    doc.autoTable({
    ...
    doc.save(`IWB_InvestorIncome_${year}.pdf`);
  };

  return (
    <div style={{ padding: '2rem', fontFamily: 'Arial, sans-serif' }}>
      <h1>📊 Investor Dashboard – Income Only</h1>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: '1rem' }}>
        ...
        <tbody>
          ...
          <tr style={{ fontWeight: 'bold', background: '#e8f4ea' }}>
            ...
            <td style={tdStyle}>{totalIncome}</td>
          </tr>
        </tbody>
      </table>
